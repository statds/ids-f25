...
The **default environment** supports all chapters except the one that requires
deep-learning libraries. The **deep-learning environment** is separate because
...
version used for this was 3.13):

```
python3.13 -m venv .ids-f25
```

Activate it:

```
. .ids-f25/bin/activate
```
...
When activated, your shell prompt begins with `(.ids-f25)`, and Python uses the
...
Most chapters are to be rendered under the `.ids-f25` virtual
environment:
...
you need to save your API key in a file named `gmapi.txt` in the
