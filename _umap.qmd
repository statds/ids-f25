...
For each point we find the nearest neighbors. The `nneighbors` hyperparameter controls
...
```{python}
#| fig-cap: "Graph with combined edge weights"
#| echo: false
#| message: false
#| warning: false
import matplotlib.pyplot as plt
...
# Compute sigma for fuzzy weights
sigma = np.mean(distances[:, 1:])
...
for i, (xi, yi) in enumerate(X):
    for j in range(1, k+1):  # skip self
        ...
        d = distances[i, j]
        w = np.exp(-d / sigma)  # fuzzy weight
        ax.plot([xi, xj], [yi, yj], color='blue', alpha=w, lw=w*3, zorder=1)  
