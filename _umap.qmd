...
utilized for clustering, data visualization, and data exploration.
...
To compute the similarity between points in the low-dimensional 
embedding, UMAP defines a corresponding low-dimensional weight 
...
- $\|y_i - y_j\|$ is the Euclidean distance between points 
  $y_i$ and $y_j$ in the low-dimensional space (2D or 3D),
- $a$ and $b$ are hyperparameters that control the shape 
  of the attraction/repulsion curve.  
...
1. If two points are **strongly connected** in high dimensions 
  (large $w_{ij}$), the embedding tries to place them **close together**.  
1. If two points are **weakly connected** (small $w_{ij}$), the embedding tries 
...
### Practical Example: UMAP on 28x28 digit data
First, install the necessary dependency:

`pip install umap-learn`


Before applying UMAP, let’s look at the raw data.
The MNIST dataset from scikit-learn contains 70,000 images of 
handwritten digits, each represented as a 28×28 grayscale 
image (784 features per sample).
```{python}
#| echo: true
#| message: false
#| warning: false
#| fig-cap: "Handwritten digit images (28×28) from MNIST dataset"
import matplotlib.pyplot as plt
import numpy as np
...
```
We now project the 784-dimensional data into a 2D embedding using UMAP.
...
#| message: false
#| warning: false
#| fig-cap: "2D UMAP of MNIST dataset"
...
effectively captures both **local and global structure** in 
high-dimensional data. By constructing a fuzzy neighborhood graph 
and optimizing a low-dimensional embedding, it preserves cluster 
relationships, highlights visually similar points, and maintains 
overall manifold structure. This makes UMAP a powerful, interpretable 
tool for **visualization and exploratory data analysis**.

### Further Readings

- [UMAP Paper](https://par.nsf.gov/biblio/10104557)  
