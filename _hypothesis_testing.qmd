...
- In the context of housing data, we can test whether the average 
house price differs from a target or benchmark value. 
...
alpha = 0.05
if p_value < alpha:
    print(f"Reject H₀ (p = {p_value:.3f}) → Mean home value differs from $20,000.")
else:
    print(f"Fail to reject H₀ (p = {p_value:.3f}) → No difference from $20,000.")
```

- The output (6.19, 1.21e-09) corresponds to the t-statistic and
p-value 
- The t-statistic means that the sample is roughly 6 standard errors 
away from the hypothesized population mean of $20,000 
...
housing prices are not centered around $20,000 
...
shown by the red dashed line at μ = 20 (\$20,000).
- Most home fall to the left of this line, showing that the actual 
mean is significantly higher than $20,000, which is consistent 
...
alpha = 0.05
if p_value < alpha:
    print(f"Reject H₀ (p = {p_value:.4f}) → Median home value differs from $20,000.")
else:
    print(f"Fail to reject H₀ (p = {p_value:.4f}) → No difference from $20,000.")
```
...
- We would reject the null hypothesis and conclude the median home 
value is significantly higher than $20,000, confiming the t-test 
results without assuming normality
...
- Here, we’ll test whether the average home value differs between 
houses located along the Charles River and those not near the river.
- H₀: μ₁ = μ₂ (the mean home values are the same for houses near and 
away from the Charles River.)
- H₁: μ₁ ≠ μ₂ (the mean home values differ depending on proximity 
...
alpha = 0.05
if p_value < alpha:
    print(f"Reject H₀ (p = {p_value:.4f}) → Mean home values differ based on proximity to the Charles River.")
else:
    print(f"Fail to reject H₀ (p = {p_value:.4f}) → No difference in mean home values based on proximity to the Charles River.")
```
...
equal variances
- t = 3.11, p = 0.0036 
- We would reject the null hypothesis so we can conclude that homes 
near the Charles River have significantly different mean values 
than those farther away 

- We can visualize these differences using boxplots 
- The boxes represent interquartile ranges, so if they barely overlap 
that's a visual clue of statisticla difference 
...
- This boxplot compares the median home values for houses next to 
the Charles River (chas =1 ) versus those not near the river 
(chas = 0) 
- The orange box is visibly higher than the blue box, showing 
that homes near the Charles river tend to have high median values 
...
if p_value < alpha:
    print(f"Reject H₀ (p = {p_value:.4f}) → "
          f"Median home values differ based on proximity to the Charles River.")
else:
    print(f"Fail to reject H₀ (p = {p_value:.4f}) → "
          f"No difference in median home values based on proximity to the Charles River.")
```
...
- Here, we’ll compare the average home values (medv) across different 
...
if p_value < alpha:
    print(f"Reject H₀ (p = {p_value:.4f}) → "
          f"Mean home values differ across highway accessibility levels.")
else:
    print(f"Fail to reject H₀ (p = {p_value:.4f}) → "
          f"No difference in mean home values across highway accessibility levels.")
...
if p_value < alpha:
    print(f"Reject H₀ (p = {p_value:.4f}) → "
          f"Median home values differ across highway accessibility levels.")
else:
    print(f"Fail to reject H₀ (p = {p_value:.4f}) → "
          f"No difference in median home values across highway accessibility levels.")
```
...
- Here, we test whether there is a linear relationship between the 
...
alpha = 0.05
if p_value < alpha:
    print(f"Reject H₀ (p = {p_value:.4f}) → There is a significant linear correlation.")
else:
    print(f"Fail to reject H₀ (p = {p_value:.4f}) → No significant correlation.")
```
...
- The Spearman correlation coefficient (ρ) measures the strength 
...
linear relationship.
- Here, we test whether the number of rooms (rm) and median home value
(medv) are 
monotonically related.
- H₀: ρ = 0 (There is no monotonic relationship between the number 
of rooms and home value.)
- H₁: ρ ≠ 0 (There is a monotonic relationship between the number 
of rooms and home value.)
...
alpha = 0.05
if p_value < alpha:
    print(f"Reject H₀ (p = {p_value:.4f}) → There is a significant monotonic relationship.")
else:
    print(f"Fail to reject H₀ (p = {p_value:.4f}) → No significant monotonic relationship.")
```
