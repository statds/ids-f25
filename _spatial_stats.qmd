...
Spatially dependent data can cause problems for standard statistical methods. If location matters, then close data points will be more similar than far data points, but standard methods often assume the data are independent.
...
PySAL offers strong tools for exploratory spatial data analysis and visualization. PyKrige lets you drop in spatial predictors or post-processing.

Spatial analysis is flexible. It can augment an existing model if you train on the residuals, or be included as a predictor when creating a new model. 

This being said, you need to validate the method. I initially wanted to demonstrate with the NYC collision data, but it turned out that crashes are independent enough for spatial patterns not to matter.
