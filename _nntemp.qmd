...
### Example: Forecasting a Synthetic Sequential Signal (PyTorch)

To compare recurrent architectures in a reproducible way, we use a
synthetic sine-wave signal with random noise. This allows us to train
RNN, LSTM, and GRU models side-by-side without large datasets or
...
```{python}
#| label: synth-data
#| fig-cap: "Synthetic sine-wave signal with random noise."
#| fig-width: 6
#| fig-height: 3
import numpy as np
import pandas as pd
...
Each training example uses the previous 20 observations to predict the
next value.
...
```{python}
#| label: synth-plot
#| fig-cap: "Observed and predicted values using RNN, LSTM, and GRU (PyTorch)."
#| fig-width: 6.5
#| fig-height: 4
...
All three networks capture the oscillatory pattern, but the vanilla RNN
has difficulty preserving phase alignment when the sequence is long.
Both the LSTM and GRU learn the dependency structure more reliably. The
GRU reaches nearly the same accuracy as the LSTM while training faster,
thanks to its simpler gating design.
