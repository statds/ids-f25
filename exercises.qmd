...
1. **NYC Crash Data Exploration** Except for the first question, use
   the cleaned crash data in feather format.
    a.  Construct a contigency table for missing in zip code but not
     in geocode (latitude and longitude) by borough.
	 Is the missing pattern the same across boroughs?
