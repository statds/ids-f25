...
1. **Quarto and Git setup**
  ...
  the [`templates/hw.qmd`](https://github.com/statds/ids-f25/blob/main/templates/hw.qmd)
  ...
  interface.
    ...
    Positron, Emacs, etc.), follow the instructions to reproduce the example
    of line plot on polar axis.
    a.  Render the homework into a HTML.
	a.  Print the HTML file to a pdf file and put the file into
...
1. **Working on Homework Problems**
  ...
  these questions help you to understand them.
    a.  Why is the command line interface if preferred among the
  professionals?
    a.  What are the advantages of Linux over Windows?
    ...
    a.  Why do I require at least 10 commits for each assignment?
...
1. **Monty Hall**
   ...
   both with 1000 trials. Summarize your results.
    a.  Explain in words which strategy is preferred. Use analytic
   results to help if you could derive them.
...
1. **Game 24**
   ...
   is one way to group the the operations.
    ...
    a.  Test your function with $(1, 3, 9, 10)$, $(4, 4, 10, 10)$,
	$(1, 5, 5, 5)$, $(3, 3, 7, 7)$, $(3, 3, 8, 8)$, and $(1, 4, 5, 6).
...
1. **NYC Crash Data Cleaning**
   ...
   The raw data needs some cleaning. The data of the Labor Day week,
   2025, was downloaded by filtering with `CRASH Date` in between
   11:59:59 PM August 30 and 12:00:00 AM September 7, available in
   `data/nyc_crashes_lbdwk_2025.csv`.
    a.  Read the data into a Pandas data frame with careful
   ...
   filter we intented. Remove the extra rows if needed.	
    ...
    a.  Provided an opportunity to meet the data curator, what
    suggestions would you make based on your data exploration
    experience?

1. **NYC Crash Data Exploration** In the 
    [cleaned crash data](data/nyc_crashes_cleaned.feather) in feather
    format (thank Wilson Tang for [his work](docs/nyc_crashes_cleaning.pdf), 
	we have a variable `zip_filled` which is `true` if, in the raw
    data, a crash has nonmissing geocode but missing zip code, which
    was filled during the cleaning process. This variable
    could be viewed as a data quality metric.
    a.  Construct a contigency table for `zip_filled` and
    `borough`.  Is the pattern of fillable zips the same across
    boroughs?  Formulate a hypothesis and test it. 
    a.  Construct an `hour` variable with integer values from 0 to 23. Plot the
...
1. **Midterm project: Noise complaints in NYC**
   The [NYC Open Data of 311 Service Requests](https://data.cityofnewyork.us/Social-Service
s/311-Service-Requests-from-2010-to-Present/erm2-nwe9) contains
   all requests from 2010 to present. We consider a subset of it with
   requests to NYPD on noise complaints that are created from
   12:00:00 AM 06/29/2025 to 12:00:00 PM 07/05/2025. The subset is
   available in CSV format as
   `data/nypd311w062925noise_by100625.csv`. 
   Read the data dictionary online to understand the meaning of the
   variables. **For a clean workflow, this data is not to be uploaded
   to your GitHub repo.** You will lose style points if you do. 
   Instead, it should be placed in an untracked
   local folder `data`, so your analysis is reproducible as long as I
   have the same folder on my computer. 
    a.  Data cleaning.
    ...
    a.  Data analysis.
       ...
       from each request incidence to the nearest police precinct. 
        i.  Add zip code level variables by merging with data from
		US Census.
