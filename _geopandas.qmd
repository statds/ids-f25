...
`centroid_ll` and `poly` are in the same CRS by setting both to EPSG:4326, which gives Latitude and Longitude.
...
`crash_df` is our `pandas.DataFrame` which contains collision data, and `zip_gdf` is our
`GeoPandas.GeoDataFrame` which contains ZIP code data.
...
Latitude and Longitude information. Function `gpd.points_from_xy()` can take
these inputs (which use EPSG:4326) to produce a `GeoSeries` of geometric 
